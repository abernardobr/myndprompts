export default {
  // App
  app: {
    name: 'MyndPrompts',
    tagline: 'Ihr KI-Prompt-Verwaltungsstudio',
  },

  // Welcome screen
  welcome: {
    title: 'MyndPrompts',
    subtitle: 'Ihr KI-Prompt-Verwaltungsstudio',
    start: 'Starten',
    newPrompt: 'Neuer Prompt',
    openPrompt: 'Prompt Öffnen',
    recent: 'Zuletzt',
    noRecentFiles: 'Keine kürzlichen Dateien',
    keyboardShortcuts: 'Tastaturkürzel',
    snippetTriggers: 'Snippet-Auslöser',
    snippetTriggersDesc: 'Im Editor eingeben zum Einfügen',
  },

  // Common actions
  common: {
    save: 'Speichern',
    cancel: 'Abbrechen',
    delete: 'Löschen',
    edit: 'Bearbeiten',
    create: 'Erstellen',
    search: 'Suchen',
    loading: 'Laden...',
    noResults: 'Keine Ergebnisse gefunden',
    close: 'Schließen',
    open: 'Öffnen',
    rename: 'Umbenennen',
    copy: 'Kopieren',
    paste: 'Einfügen',
    cut: 'Ausschneiden',
    undo: 'Rückgängig',
    redo: 'Wiederholen',
    selectAll: 'Alles Auswählen',
    refresh: 'Aktualisieren',
    add: 'Hinzufügen',
    remove: 'Entfernen',
    yes: 'Ja',
    no: 'Nein',
    ok: 'OK',
    confirm: 'Bestätigen',
    back: 'Zurück',
    next: 'Weiter',
    finish: 'Fertig',
    apply: 'Anwenden',
    reset: 'Zurücksetzen',
    clear: 'Leeren',
    filter: 'Filtern',
    sort: 'Sortieren',
    view: 'Ansicht',
    help: 'Hilfe',
    about: 'Über',
    version: 'Version',
    error: 'Fehler',
    warning: 'Warnung',
    info: 'Information',
    success: 'Erfolg',
    retry: 'Erneut versuchen',
    item: 'Element',
    items: 'Elemente',
  },

  // Activity bar
  activityBar: {
    explorer: 'Explorer',
    search: 'Suchen',
    snippets: 'Snippets',
    favorites: 'Favoriten',
    sourceControl: 'Quellcodeverwaltung',
    settings: 'Einstellungen',
  },

  // Sidebar panels
  sidebar: {
    explorer: 'Explorer',
    search: 'Suchen',
    snippets: 'Bibliothek',
    favorites: 'Favoriten',
    sourceControl: 'Quellcodeverwaltung',
    settings: 'Einstellungen',
  },

  // Explorer panel
  explorer: {
    title: 'Explorer',
    prompts: 'Prompts',
    snippets: 'Snippets',
    projects: 'Projekte',
    searchPlaceholder: 'Prompts suchen...',
    allCategories: 'Alle Kategorien',
    noPrompts: 'Noch keine Prompts',
    noPromptsHint: 'Klicken Sie auf +, um Ihren ersten Prompt zu erstellen',
    noMatchingPrompts: 'Keine Prompts entsprechen Ihrer Suche',
    newPrompt: 'Neuer Prompt',
    newProject: 'Neues Projekt',
    newProjectLibrary: 'Neue Projektbibliothek',
    newDirectory: 'Neuer Ordner',
    exportLibrary: 'Bibliothek Exportieren',
    openInEditor: 'Im Editor Öffnen',
    addToFavorites: 'Zu Favoriten Hinzufügen',
    removeFromFavorites: 'Aus Favoriten Entfernen',
    copyContent: 'Inhalt Kopieren',
    copyPath: 'Pfad Kopieren',
    revealInFinder: 'Im Finder Anzeigen',
    revealInExplorer: 'Im Explorer Anzeigen',
    deletePrompt: 'Prompt Löschen',
    deleteProject: 'Projekt Löschen',
    deleteDirectory: 'Ordner Löschen',
    collapseAll: 'Alle Einklappen',
    expandAll: 'Alle Ausklappen',
    toggleFavorite: 'Favorit umschalten',
    initializePersonas: 'Personas initialisieren',
    ignore: 'Ignorieren',
  },

  // Search panel
  searchPanel: {
    title: 'Suchen',
    placeholder: 'In Prompts und Snippets suchen...',
    matchCase: 'Groß-/Kleinschreibung',
    matchWholeWord: 'Ganzes Wort',
    useRegex: 'Regulären Ausdruck Verwenden',
    searchIn: 'Suchen In',
    prompts: 'Prompts',
    snippets: 'Snippets',
    all: 'Alle',
    results: 'Ergebnisse',
    noResults: 'Keine Ergebnisse gefunden',
    searching: 'Suche läuft...',
    filterByCategories: 'Nach Kategorien filtern',
    enterSearchTerm: 'Geben Sie einen Suchbegriff ein, um Prompts und Snippets zu finden',
    noResultsFor: 'Keine Ergebnisse gefunden für "{query}"',
    resultCount: '{count} Ergebnis | {count} Ergebnisse',
    prompt: 'Prompt',
    snippet: 'Snippet',
    matchInFilename: 'Übereinstimmung im Dateinamen: {filename}',
    uncategorized: 'Unkategorisiert',
  },

  // Snippets panel
  snippetsPanel: {
    title: 'Snippets',
    searchPlaceholder: 'Bibliothek durchsuchen...',
    createNew: 'Neues Snippet erstellen',
    personas: 'Personas',
    textSnippets: 'Text-Snippets',
    codeSnippets: 'Code-Snippets',
    templates: 'Vorlagen',
    noSnippets: 'Noch keine Snippets',
    noSnippetsHint: 'Klicken Sie auf +, um Ihr erstes Snippet zu erstellen',
    noMatchingSnippets: 'Keine Snippets entsprechen Ihrer Suche',
    noCategorySnippets: 'Keine Snippets in dieser Kategorie',
    copyShortcut: 'Kürzel kopieren',
    copyContent: 'Inhalt kopieren',
    insertTip: "Geben Sie {'@'} im Editor ein, um Snippets einzufügen",
    triggerHelp: {
      title: 'Snippets im Editor Einfügen',
      all: 'Alle Snippets (universal)',
      text: 'Nur Text-Snippets',
      code: 'Nur Code-Snippets',
      templates: 'Nur Vorlagen',
      files: 'Projektdateien',
      tip: 'Beginnen Sie nach dem Auslöser zu tippen, um Ergebnisse zu filtern',
    },
    browserMode: 'Das Snippet-Panel ist in der Desktop-App verfügbar.',
    browserModeHint: 'Läuft im Browser-Vorschaumodus.',
    filterByTags: 'Nach Tags filtern...',
    noTagsFound: 'Keine Tags gefunden',
    filterByLanguage: 'Sprache',
    selectLanguage: 'Sprache auswählen...',
    allLanguages: 'Alle Sprachen',
    noLanguagesFound: 'Keine Sprachen gefunden',
  },

  // Favorites panel
  favoritesPanel: {
    title: 'Favoriten',
    searchPlaceholder: 'Favoriten suchen...',
    categories: 'Kategorien',
    clearFilter: 'Filter löschen',
    noFavorites: 'Noch keine Favoriten.',
    noMatchingFavorites: 'Keine Favoriten entsprechen Ihrer Suche.',
    noCategoryFavorites: 'Keine Favoriten in dieser Kategorie.',
    addHint:
      'Klicken Sie mit der rechten Maustaste auf einen Prompt im Explorer und wählen Sie "Zu Favoriten Hinzufügen"',
    removeFromFavorites: 'Aus Favoriten entfernen',
    copyContent: 'Inhalt kopieren',
    favoriteCount: '{count} Favorit | {count} Favoriten',
    browserMode: 'Das Favoriten-Panel ist in der Desktop-App verfügbar.',
    browserModeHint: 'Läuft im Browser-Vorschaumodus.',
  },

  // Git panel
  gitPanel: {
    title: 'Quellcodeverwaltung',
    notInitialized: 'Git nicht initialisiert',
    initializeRepo: 'Repository Initialisieren',
    stagedChanges: 'Bereitgestellte Änderungen',
    changes: 'Änderungen',
    untrackedFiles: 'Nicht verfolgte Dateien',
    noChanges: 'Keine Änderungen',
    commitMessage: 'Commit-Nachricht',
    commitPlaceholder: 'Nachricht (Enter drücken zum Committen)',
    commit: 'Committen',
    push: 'Pushen',
    pull: 'Pullen',
    fetch: 'Abrufen',
    sync: 'Synchronisieren',
    stash: 'Stashen',
    stashPop: 'Stash Wiederherstellen',
    branch: 'Branch',
    branches: 'Branches',
    currentBranch: 'Aktueller Branch',
    newBranch: 'Neuer Branch',
    switchBranch: 'Branch Wechseln',
    deleteBranch: 'Branch Löschen',
    mergeBranch: 'Branch Zusammenführen',
    stageAll: 'Alle Bereitstellen',
    unstageAll: 'Alle Zurückstellen',
    discardAll: 'Alle Verwerfen',
    stage: 'Bereitstellen',
    unstage: 'Zurückstellen',
    discard: 'Verwerfen',
    viewHistory: 'Verlauf Anzeigen',
    setupGit: 'Git Einrichten',
    gitNotInstalled: 'Git ist nicht installiert',
    browserMode: 'Quellcodeverwaltung ist in der Desktop-App verfügbar.',
    browserModeHint: 'Läuft im Browser-Vorschaumodus.',
  },

  // Settings panel
  settingsPanel: {
    title: 'Einstellungen',
    appearance: 'Erscheinungsbild',
    theme: 'Design',
    language: 'Sprache',
    promptCategories: 'Prompt-Kategorien',
    editor: 'Editor',
    fontSize: 'Schriftgröße',
    wordWrap: 'Zeilenumbruch',
    storage: 'Speicher',
    storageLocation: 'Speicherort',
    openFullSettings: 'Vollständige Einstellungen Öffnen',
    addCategory: 'Kategorie Hinzufügen',
    removeCategory: 'Kategorie Entfernen',
    categoryName: 'Kategoriename',
    categoryValue: 'Kategoriewert',
    on: 'Ein',
    off: 'Aus',
    resetToDefaults: 'Zurücksetzen',
    resetToDefaultsTooltip: 'Kategorien auf Standardwerte zurücksetzen',
    newCategoryPlaceholder: 'Name der neuen Kategorie',
    loadingCategories: 'Kategorien werden geladen...',
    categoryRequired: 'Kategoriename ist erforderlich',
    categoryTooLong: 'Kategoriename darf maximal 50 Zeichen lang sein',
    failedToAdd: 'Kategorie konnte nicht hinzugefügt werden',
    failedToUpdate: 'Kategorie konnte nicht aktualisiert werden',
    failedToDelete: 'Kategorie konnte nicht gelöscht werden',
    failedToReset: 'Kategorien konnten nicht zurückgesetzt werden',
  },

  // Settings (export/import)
  settings: {
    storage: {
      browserMode: 'Die Speichereinstellungen sind in der Desktop-App verfügbar.',
      locationHint:
        'Alle Ihre Prompts, Snippets und Einstellungen werden an diesem Ort gespeichert.',
      openInExplorer: 'Im Datei-Explorer öffnen',
      exportTitle: 'Daten Exportieren',
      exportDescription:
        'Exportieren Sie alle Ihre Prompts, Snippets, Personas und Vorlagen als ZIP-Archiv.',
      exportButton: 'Alle Daten Exportieren',
      exportDialogTitle: 'MyndPrompts Daten Exportieren',
      exportSuccess: 'Daten erfolgreich exportiert',
      exportError: 'Fehler beim Exportieren der Daten',
      exportSummary: '{prompts} Prompts, {snippets} Snippets, {total} Dateien insgesamt exportiert',
      exporting: 'Exportiere...',
      importTitle: 'Daten Importieren',
      importDescription:
        'Importieren Sie Prompts und andere Daten aus einem zuvor exportierten ZIP-Archiv.',
      importWarning:
        'Vorhandene Dateien mit demselben Namen werden umbenannt, um Konflikte zu vermeiden.',
      importButton: 'Daten Importieren',
      importDialogTitle: 'MyndPrompts Daten Importieren',
      importSuccess: 'Daten erfolgreich importiert',
      importError: 'Fehler beim Importieren der Daten',
      importSummary:
        '{prompts} Prompts, {snippets} Snippets, {total} Dateien insgesamt importiert ({skipped} übersprungen)',
      importing: 'Importiere...',
      // Error messages - keyed by ERROR_CODES
      errors: {
        FILE_NOT_FOUND: 'Datei nicht gefunden',
        INVALID_ZIP: 'Ungültiges ZIP-Dateiformat',
        MISSING_MANIFEST: 'Fehlende oder ungültige Manifest-Datei',
        INVALID_MANIFEST: 'Die Manifest-Datei ist fehlerhaft oder ungültig',
        VERSION_INCOMPATIBLE: 'Dieser Export wurde mit einer inkompatiblen Version erstellt',
        WRITE_ERROR: 'Fehler beim Schreiben zum Ziel',
        PERMISSION_DENIED: 'Zugriff verweigert - überprüfen Sie die Zugriffsrechte',
        CORRUPTED_ZIP: 'Die ZIP-Datei ist beschädigt oder kann nicht extrahiert werden',
        EMPTY_FILE: 'Die Datei ist leer',
        FILE_TOO_LARGE: 'Die Dateigröße überschreitet das maximal zulässige Limit',
        CHECKSUM_MISMATCH: 'Integritätsprüfung fehlgeschlagen - Daten könnten beschädigt sein',
        UNKNOWN_ERROR: 'Ein unerwarteter Fehler ist aufgetreten',
        // Legacy keys for backward compatibility
        invalidZip: 'Ungültiges ZIP-Dateiformat',
        missingManifest: 'Fehlende oder ungültige Manifest-Datei',
        versionIncompatible: 'Inkompatible Export-Version',
        fileNotFound: 'Datei nicht gefunden',
        permissionDenied: 'Zugriff verweigert',
      },

      // Progress phases
      progress: {
        reading: 'Dateien werden gelesen',
        compressing: 'Komprimieren',
        writing: 'Archiv wird geschrieben',
        extracting: 'Extrahieren',
        validating: 'Validieren',
        copying: 'Dateien werden kopiert',
      },

      // Move Storage
      moveStorage: {
        sectionTitle: 'Speicherort Verschieben',
        sectionDescription:
          'Verschieben Sie Ihre Prompts, Snippets und andere Daten an einen anderen Ort auf Ihrem Computer.',
        button: 'Speicherort Verschieben',
        successNotification: 'Speicher erfolgreich verschoben',
        title: 'Speicherort Verschieben',
        description:
          'Verschieben Sie alle Ihre Daten an einen neuen Ort. Alle Dateien werden kopiert und die Konfiguration wird aktualisiert.',
        currentLocation: 'Aktueller Speicherort',
        newLocation: 'Neuer Speicherort',
        noFolderSelected: 'Kein Ordner ausgewählt',
        selectFolder: 'Ordner Auswählen',
        validDestination: 'Gültiges Ziel',
        available: 'verfügbar',
        confirmTitle: 'Migration Bestätigen',
        confirmMessage: 'Die folgenden Daten werden an den neuen Speicherort verschoben:',
        migrationPlan: 'Migrationsplan',
        totalFiles: 'Dateien Gesamt',
        totalSize: 'Gesamtgröße',
        directories: 'Verzeichnisse',
        warningMessage:
          'Dieser Vorgang kann nicht automatisch rückgängig gemacht werden. Stellen Sie sicher, dass Sie ein Backup Ihrer Daten haben.',
        progressTitle: 'Speicher Wird Verschoben',
        fileProgress: '{copied} von {total} Dateien',
        bytesProgress: '{copied} von {total}',
        copyingDirectory: 'Kopiere: {directory}',
        completeTitle: 'Migration Abgeschlossen',
        completeMessage: 'Ihr Speicher wurde erfolgreich an den neuen Ort verschoben.',
        summary: 'Zusammenfassung',
        filesCopied: 'Dateien Kopiert',
        dataCopied: 'Daten Kopiert',
        duration: 'Dauer',
        newStoragePath: 'Neuer Speicherpfad',
        cleanupTitle: 'Bereinigung',
        cleanupMessage: 'Möchten Sie die Dateien vom alten Speicherort löschen?',
        deleteOldFiles: 'Alte Dateien Löschen',
        keepOldFiles: 'Alte Dateien Behalten',
        errorTitle: 'Migration Fehlgeschlagen',
        errorMessage: 'Während des Migrationsprozesses ist ein Fehler aufgetreten.',
        errorDetails: 'Fehlerdetails',
        cancel: 'Abbrechen',
        back: 'Zurück',
        next: 'Weiter',
        startMigration: 'Migration Starten',
        close: 'Schließen',
        retry: 'Erneut Versuchen',
        errorNotification: 'Speichermigration fehlgeschlagen',
        cancelled: 'Migration abgebrochen',
        cleanupSuccess: 'Alte Dateien erfolgreich gelöscht',
        cleanupError: 'Fehler beim Löschen alter Dateien',
        destinationNotEmpty: 'Der Zielordner muss leer sein',
        destinationInsideSource: 'Das Ziel kann nicht innerhalb des aktuellen Speichers liegen',
        noWritePermission: 'Keine Schreibberechtigung für den ausgewählten Ordner',
        notEnoughSpace: 'Nicht genügend Speicherplatz verfügbar',
        phase: {
          validating: 'Validiere...',
          planning: 'Migration wird geplant...',
          copying: 'Dateien werden kopiert...',
          verifying: 'Verifiziere...',
          completing: 'Abschließen...',
        },
        currentFile: 'Aktuelle Datei',
        move: 'Speicher verschieben',
        successMessage: 'Ihr Speicher wurde erfolgreich an den neuen Speicherort verschoben.',
        verificationSuccess: 'Alle Dateien erfolgreich verifiziert',
        verificationWarning: 'Einige Dateien konnten nicht verifiziert werden',
        cleanupWarning: 'Diese Aktion kann nicht rückgängig gemacht werden',
        deleteOldStorage: 'Alten Speicher löschen',
        keepOldStorage: 'Alte Dateien behalten',
        partialMigration:
          'Ihre Originaldateien befinden sich noch am Quellort. Sie können es erneut versuchen oder sie als Backup behalten.',
        partialMigrationInfo: '{copied} Dateien wurden vor dem Fehler kopiert.',
        rollbackTitle: 'Migration rückgängig machen',
        rollbackDescription:
          'Die teilweise kopierten Dateien vom Ziel entfernen. Ihre Originaldateien am Quellort bleiben erhalten.',
        rollbackButton: 'Änderungen rückgängig machen',
        rollbackSuccess: 'Rückgängig gemacht. {count} Dateien entfernt.',
        rollbackError: 'Fehler beim Rückgängigmachen der Migration',
        errors: {
          diskFull: 'Festplatte voll',
          permissionDenied: 'Zugriff verweigert',
          fileInUse: 'Datei in Verwendung',
          validationFailed: 'Validierung fehlgeschlagen',
          copyFailed: 'Kopieren fehlgeschlagen',
          verificationFailed: 'Verifizierung fehlgeschlagen',
        },
      },
    },
  },

  // Theme
  theme: {
    light: 'Hell',
    dark: 'Dunkel',
    system: 'System',
  },

  // Languages
  languages: {
    'en-US': 'Englisch (USA)',
    'en-GB': 'Englisch (Vereinigtes Königreich)',
    'en-IE': 'Englisch (Irland)',
    'pt-BR': 'Portugiesisch (Brasilien)',
    'pt-PT': 'Portugiesisch (Portugal)',
    'es-ES': 'Spanisch (Spanien)',
    'fr-FR': 'Französisch (Frankreich)',
    'de-DE': 'Deutsch (Deutschland)',
    'it-IT': 'Italienisch (Italien)',
    'ar-SA': 'Arabisch (Saudi-Arabien)',
  },

  // Bottom panel
  bottomPanel: {
    output: 'Ausgabe',
    problems: 'Probleme',
    gitChanges: 'Git-Änderungen',
    aiChat: 'KI-Chat',
    maximize: 'Panel Maximieren',
    restore: 'Panel Wiederherstellen',
    close: 'Panel Schließen',
  },

  // Prompts
  prompts: {
    title: 'Prompts',
    new: 'Neuer Prompt',
    open: 'Prompt Öffnen',
    save: 'Prompt Speichern',
    saveAs: 'Speichern Unter...',
    favorites: 'Favoriten',
    recent: 'Zuletzt',
    all: 'Alle Prompts',
    untitled: 'Unbenannt',
    modified: 'Geändert',
    category: 'Kategorie',
    tags: 'Tags',
    description: 'Beschreibung',
    content: 'Inhalt',
    metadata: 'Metadaten',
    createdAt: 'Erstellt am',
    updatedAt: 'Aktualisiert am',
  },

  // Dialogs
  dialogs: {
    newPrompt: {
      title: 'Neuer Prompt',
      promptTitle: 'Titel',
      titlePlaceholder: 'Prompt-Titel eingeben...',
      category: 'Kategorie',
      categoryOptional: 'Kategorie (optional)',
      selectCategory: 'Kategorie auswählen',
      titleRequired: 'Titel ist erforderlich',
      titleTooLong: 'Titel muss 100 Zeichen oder weniger sein',
    },
    openPrompt: {
      title: 'Prompt Öffnen',
      selectFile: 'Wählen Sie eine Prompt-Datei zum Öffnen',
      recentFiles: 'Zuletzt Verwendete Dateien',
      browse: 'Durchsuchen...',
    },
    newSnippet: {
      title: 'Neues Snippet',
      name: 'Name',
      namePlaceholder: 'Snippet-Name eingeben...',
      type: 'Typ',
      description: 'Beschreibung',
      descriptionPlaceholder: 'Optionale Beschreibung...',
      persona: 'Persona',
      personaDesc: 'KI-Personas und -Rollen definieren',
      textSnippet: 'Text-Snippet',
      textSnippetDesc: 'Wiederverwendbare Textblöcke',
      codeSnippet: 'Code-Snippet',
      codeSnippetDesc: 'Code-Vorlagen und -Beispiele',
      template: 'Vorlage',
      templateDesc: 'Vollständige Prompt-Vorlagen',
      tags: 'Tags',
      tagsPlaceholder: 'Tag hinzufügen...',
      tagsHint: 'Wählen Sie vorhandene Tags oder tippen Sie, um neue zu erstellen',
      tagsEmpty: 'Noch keine Tags',
      language: 'Sprache',
      languageHint: 'Wählen Sie die Inhaltssprache',
    },
    newProject: {
      title: 'Neues Projekt',
      name: 'Projektname',
      namePlaceholder: 'Projektnamen eingeben...',
      description: 'Beschreibung',
      descriptionPlaceholder: 'Optionale Projektbeschreibung...',
    },
    newProjectLibrary: {
      title: 'Neue Projektbibliothek',
    },
    editPrompt: {
      tags: 'Tags',
      tagsPlaceholder: 'Tag hinzufügen und Enter drücken',
      language: 'Sprache',
    },
    exportLibrary: {
      title: 'Bibliothek Exportieren',
      description:
        'Exportieren Sie dieses Projekt als JSON-Bibliotheksdatei, die geteilt und importiert werden kann.',
      name: 'Bibliotheksname',
      namePlaceholder: 'Bibliotheksnamen eingeben...',
      libraryDescription: 'Beschreibung',
      descriptionPlaceholder: 'Beschreiben Sie, was diese Bibliothek enthält...',
      language: 'Sprache',
      destinationFolder: 'Zielordner',
      destinationFolderPlaceholder: 'Ordner auswählen...',
      selectFolder: 'Zielordner Auswählen',
      export: 'Exportieren',
      saveDialogTitle: 'Bibliothek Speichern',
      success: 'Bibliothek erfolgreich exportiert mit {count} Elementen',
      noFilesFound: 'Keine Dateien zum Exportieren in diesem Projekt gefunden',
    },
    newDirectory: {
      title: 'Neuer Ordner',
      name: 'Ordnername',
      namePlaceholder: 'Ordnernamen eingeben...',
    },
    rename: {
      title: 'Umbenennen',
      newName: 'Neuer Name',
      namePlaceholder: 'Neuen Namen eingeben...',
      renamePrompt: 'Prompt Umbenennen',
      renameSnippet: 'Snippet Umbenennen',
      renameProject: 'Projekt Umbenennen',
      renameDirectory: 'Ordner Umbenennen',
    },
    delete: {
      title: 'Löschen',
      confirmTitle: 'Löschen Bestätigen',
      confirmMessage: 'Sind Sie sicher, dass Sie "{name}" löschen möchten?',
      confirmMessageWithCount:
        'Dieser Ordner enthält {count} Elemente. Sind Sie sicher, dass Sie ihn löschen möchten?',
      cannotUndo: 'Diese Aktion kann nicht rückgängig gemacht werden.',
      deletePrompt: 'Prompt Löschen',
      deleteSnippet: 'Snippet Löschen',
      deleteProject: 'Projekt Löschen',
      deleteDirectory: 'Ordner Löschen',
    },
    update: {
      title: 'Software-Update',
      checking: 'Suche nach Updates...',
      available: 'Update verfügbar',
      upToDate: 'Sie sind auf dem neuesten Stand',
      errorTitle: 'Updateprüfung fehlgeschlagen',
      newVersion: 'Eine neue Version von MyndPrompts ist verfügbar.',
      currentVersion: 'Aktuelle Version',
      latestVersion: 'Neueste Version',
      releaseNotes: 'Versionshinweise',
      downloadBtn: 'Update herunterladen',
      remindLater: 'Später erinnern',
      skipVersion: 'Diese Version überspringen',
      close: 'Schließen',
      retry: 'Erneut versuchen',
      upToDateMessage: 'Sie verwenden die neueste Version von MyndPrompts.',
    },
    help: {
      title: 'Willkommen bei MyndPrompts',
      description:
        'Sehen Sie sich diese Tutorial-Videos an, um zu lernen, wie Sie MyndPrompts optimal nutzen können.',
      watchOnYouTube: 'Tutorial-Videos ansehen',
      clickToOpen: 'Klicken Sie, um auf YouTube zu öffnen',
      dontShowAgain: 'Beim Start nicht mehr anzeigen',
      close: 'Schließen',
    },
    gitSetup: {
      title: 'Git-Einrichtung',
      userName: 'Benutzername',
      userNamePlaceholder: 'Ihr Name',
      userEmail: 'E-Mail',
      userEmailPlaceholder: "ihre.email{'@'}beispiel.com",
      initRepository: 'Repository Initialisieren',
      configureGit: 'Git Konfigurieren',
      remoteName: 'Remote-Name',
      dangerZone: 'Gefahrenzone',
      removeGit: 'Git Entfernen',
      removeGitDescription:
        'Dadurch wird die Git-Versionskontrolle von diesem Projekt entfernt. Ihre Dateien werden nicht gelöscht, aber die gesamte Commit-Historie geht verloren.',
      removeGitConfirmTitle: 'Git-Repository Entfernen?',
      removeGitConfirmMessage:
        'Sind Sie sicher, dass Sie Git aus diesem Projekt entfernen möchten? Dadurch wird der .git-Ordner und die gesamte Commit-Historie gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.',
      gitRemoved: 'Git-Repository erfolgreich entfernt',
      gitRemoveFailed: 'Fehler beim Entfernen des Git-Repositories',
      discardChanges: 'Änderungen verwerfen',
      discardChangesTitle: 'Änderungen verwerfen?',
      discardChangesMessage: 'Sind Sie sicher, dass Sie die Änderungen verwerfen möchten an:',
      discardChangesWarning:
        'Dadurch wird die Datei auf ihren letzten commiteten Zustand zurückgesetzt. Diese Aktion kann nicht rückgängig gemacht werden.',
      restoreFile: 'Datei wiederherstellen',
    },
    branch: {
      title: 'Branches',
      newBranch: 'Neuer Branch',
      branchName: 'Branch-Name',
      branchNamePlaceholder: 'Branch-Namen eingeben...',
      switchTo: 'Wechseln zu',
      delete: 'Löschen',
      current: 'Aktuell',
      local: 'Lokal',
      remote: 'Remote',
    },
    gitHistory: {
      title: 'Git-Verlauf',
      commits: 'Commits',
      noCommits: 'Noch keine Commits',
      viewChanges: 'Änderungen Anzeigen',
      hash: 'Hash',
      author: 'Autor',
      date: 'Datum',
      message: 'Nachricht',
    },
    commitFile: {
      title: 'Datei Committen',
      message: 'Diese Datei committen?',
      enterMessage: 'Commit-Nachricht eingeben...',
    },
    discardChanges: {
      title: 'Änderungen Verwerfen',
      message: 'Sind Sie sicher, dass Sie die Änderungen in "{name}" verwerfen möchten?',
      warning: 'Diese Aktion kann nicht rückgängig gemacht werden.',
    },
    fileExists: {
      title: 'Datei Existiert Bereits',
      message: 'Eine Datei mit diesem Namen existiert bereits. Möchten Sie sie ersetzen?',
      replace: 'Ersetzen',
      keep: 'Beide Behalten',
    },
  },

  // Editor
  editor: {
    untitled: 'Unbenannt',
    unsavedChanges: 'Ungespeicherte Änderungen',
    saveChanges: 'Änderungen Speichern',
    discardChanges: 'Änderungen Verwerfen',
    closeWithoutSaving: 'Schließen ohne Speichern',
    readOnly: 'Nur Lesen',
    preview: 'Vorschau',
    edit: 'Bearbeiten',
    splitRight: 'Nach Rechts Teilen',
    splitDown: 'Nach Unten Teilen',
    closePane: 'Bereich Schließen',
    closeTab: 'Tab Schließen',
    closeOthers: 'Andere Schließen',
    closeAll: 'Alle Schließen',
    closeToRight: 'Rechts Schließen',
    pinTab: 'Tab Anheften',
    unpinTab: 'Tab Lösen',
    copyPath: 'Pfad Kopieren',
    revealInSidebar: 'In Seitenleiste Anzeigen',
    insertImage: 'Bild Einfügen',
    formatAsImage: 'Als Bild Formatieren',
  },

  // Tab bar
  tabBar: {
    close: 'Schließen',
    closeOthers: 'Andere Schließen',
    closeAll: 'Alle Schließen',
    pinTab: 'Tab Anheften',
    unpinTab: 'Tab Lösen',
    copyPath: {
      label: 'Pfad Kopieren',
      absolute: 'Absoluter Pfad',
      filename: 'Dateiname',
      relative: 'Relativer Pfad',
      absoluteCopied: 'Absoluter Pfad in die Zwischenablage kopiert',
      filenameCopied: 'Dateiname in die Zwischenablage kopiert',
      relativeCopied: 'Relativer Pfad in die Zwischenablage kopiert',
      error: 'Fehler beim Kopieren in die Zwischenablage',
    },
  },

  // Status bar
  statusBar: {
    line: 'Zl',
    column: 'Sp',
    encoding: 'UTF-8',
    lineEnding: 'LF',
    language: 'Markdown',
    notifications: 'Benachrichtigungen',
    noNotifications: 'Keine Benachrichtigungen',
  },

  // Categories
  categories: {
    general: 'Allgemein',
    development: 'Entwicklung',
    writing: 'Schreiben',
    analysis: 'Analyse',
    creative: 'Kreativ',
    business: 'Geschäft',
    personal: 'Persönlich',
    education: 'Bildung',
    research: 'Forschung',
    other: 'Sonstige',
  },

  // Validation
  validation: {
    required: 'Dieses Feld ist erforderlich',
    minLength: 'Muss mindestens {min} Zeichen haben',
    maxLength: 'Muss {max} Zeichen oder weniger haben',
    invalidFormat: 'Ungültiges Format',
    invalidEmail: 'Ungültige E-Mail-Adresse',
    invalidUrl: 'Ungültige URL',
  },

  // Errors
  errors: {
    generic: 'Ein Fehler ist aufgetreten',
    loadFailed: 'Laden fehlgeschlagen',
    saveFailed: 'Speichern fehlgeschlagen',
    deleteFailed: 'Löschen fehlgeschlagen',
    createFailed: 'Erstellen fehlgeschlagen',
    renameFailed: 'Umbenennen fehlgeschlagen',
    copyFailed: 'Kopieren fehlgeschlagen',
    networkError: 'Netzwerkfehler',
    permissionDenied: 'Zugriff verweigert',
    fileNotFound: 'Datei nicht gefunden',
    directoryNotFound: 'Ordner nicht gefunden',
    alreadyExists: 'Existiert bereits',
  },

  // Success messages
  success: {
    saved: 'Erfolgreich gespeichert',
    deleted: 'Erfolgreich gelöscht',
    created: 'Erfolgreich erstellt',
    renamed: 'Erfolgreich umbenannt',
    copied: 'In Zwischenablage kopiert',
    updated: 'Erfolgreich aktualisiert',
  },

  // Tooltips
  tooltips: {
    newFile: 'Neue Datei',
    openFile: 'Datei Öffnen',
    saveFile: 'Datei Speichern',
    refresh: 'Aktualisieren',
    collapse: 'Einklappen',
    expand: 'Ausklappen',
    more: 'Weitere Aktionen',
    close: 'Schließen',
    maximize: 'Maximieren',
    minimize: 'Minimieren',
    restore: 'Wiederherstellen',
  },

  // Keyboard shortcuts
  shortcuts: {
    newPrompt: 'Neuer Prompt',
    openFile: 'Datei Öffnen',
    save: 'Speichern',
    splitEditor: 'Editor Teilen',
    search: 'Suchen',
    settings: 'Einstellungen',
  },

  // File Sync
  fileSync: {
    title: 'Dateisynchronisierung',
    selectFolder: 'Ordner auswählen',
    folders: 'Ordner',
    addFolder: 'Ordner hinzufügen',
    removeFolder: 'Ordner entfernen',
    syncNow: 'Jetzt synchronisieren',
    syncAll: 'Alle synchronisieren',
    stopAll: 'Alle stoppen',
    cannotRemoveWhileIndexing: 'Ordner kann nicht entfernt werden, während die Indexierung läuft',
    upToDate: 'Aktuell',
    needsSync: 'Synchronisierung erforderlich',
    indexing: 'Indizierung...',
    filesIndexed: 'Dateien indiziert',
    folderCount: '{count} Ordner | {count} Ordner',
    fileCount: '{count} Datei | {count} Dateien',
    hasErrors: '{count} Synchronisierungsfehler | {count} Synchronisierungsfehler',
    cancelIndexing: 'Indizierung abbrechen',
    noFolders: 'Keine Ordner konfiguriert',
    noProject: 'Bitte wählen Sie zuerst ein Projekt',
    addFolderHint:
      'Fügen Sie externe Ordner hinzu, um Dateipfad-Vorschläge im Editor zu aktivieren',
    browserMode: 'Dateisynchronisierung ist in der Desktop-App verfügbar',
    status: {
      pending: 'Ausstehend',
      indexing: 'Indizierung',
      indexed: 'Indiziert',
      error: 'Fehler',
    },
  },

  // Libraries
  plugins: {
    // Section title
    title: 'Bibliotheken',

    // Tabs
    marketplace: 'Marketplace',
    installed: 'Installiert',

    // Search and filters
    searchPlaceholder: 'Bibliotheken suchen...',
    searchItems: 'Vorlagen, Personas, Snippets suchen...',
    noItemsFound: 'Keine Elemente gefunden',
    tryDifferentSearch: 'Versuchen Sie einen anderen Suchbegriff',
    installLibrary: 'Bibliothek installieren',
    hintNavigate: 'navigieren',
    hintInstall: 'Bibliothek installieren',
    hintUninstall: 'deinstallieren',
    hintClose: 'schließen',
    filterByType: 'Typ',
    filterByTags: 'Tags',
    allTypes: 'Alle',
    allTags: 'Alle Tags',
    clearFilters: 'Filter löschen',
    selectTags: 'Tags auswählen...',
    noTagsFound: 'Keine Tags gefunden',
    filterByLanguage: 'Sprache',
    selectLanguage: 'Sprache auswählen...',
    allLanguages: 'Alle Sprachen',
    noLanguagesFound: 'Keine Sprachen gefunden',

    // Library types
    types: {
      persona: 'Persona',
      templates: 'Vorlagen',
      code_snippets: 'Code-Snippets',
      text_snippets: 'Text-Snippets',
    },

    // Library card
    version: 'Version',
    items: 'Elemente',
    installedOn: 'Installiert am',
    updatedOn: 'Aktualisiert am',

    // Actions
    view: 'Ansehen',
    install: 'Installieren',
    installing: 'Installiere...',
    update: 'Aktualisieren',
    updateTo: 'Auf v{version} aktualisieren',
    updateAll: 'Alle aktualisieren',
    updating: 'Aktualisiere...',
    uninstall: 'Deinstallieren',
    uninstalling: 'Deinstalliere...',

    // Confirmation dialog
    uninstallTitle: 'Bibliothek deinstallieren',
    uninstallMessage: 'Möchten Sie "{name}" wirklich deinstallieren?',
    uninstallWarning: 'Dies entfernt alle von dieser Bibliothek bereitgestellten Elemente.',

    // Empty states
    noPluginsFound: 'Keine Bibliotheken gefunden',
    noPluginsFoundHint: 'Versuchen Sie, Ihre Filter anzupassen',
    noPluginsInstalled: 'Keine Bibliotheken installiert',
    noPluginsInstalledHint:
      'Besuchen Sie den Marketplace, um Bibliotheken zu entdecken und zu installieren',
    noInstalledLibraries: 'Keine installierten Bibliotheken',
    goToMarketplace: 'Wechseln Sie zum Marketplace-Tab, um Bibliotheken zu installieren',
    browseMarketplace: 'Marketplace durchsuchen',
    noPluginsOfType: 'Keine installierten Bibliotheken dieses Typs',
    installFromMarketplace: 'Installieren Sie zuerst Bibliotheken aus dem Marketplace',
    noContent: 'Kein Inhalt verfügbar',

    // Content selector dialog
    addFromLibrary: 'Aus Bibliothek hinzufügen',
    selectAll: 'Alle auswählen',
    of: 'von',
    selected: 'ausgewählt',
    addSelected: 'Ausgewählte hinzufügen',

    // Notifications
    installSuccess: 'Bibliothek erfolgreich installiert',
    installError: 'Bibliothek-Installation fehlgeschlagen',
    updateSuccess: 'Bibliothek erfolgreich aktualisiert',
    updateError: 'Bibliothek-Aktualisierung fehlgeschlagen',
    uninstallSuccess: 'Bibliothek erfolgreich deinstalliert',
    uninstallError: 'Bibliothek-Deinstallation fehlgeschlagen',
    contentAddedSuccess: '{count} Element(e) aus der Bibliothek hinzugefügt',
    contentAddedError: 'Fehler beim Hinzufügen von Elementen aus der Bibliothek',
    clearAllSuccess: 'Alle Bibliotheken erfolgreich entfernt',
    clearAllError: 'Fehler beim Löschen der Bibliotheken',

    // Clear all (troubleshooting)
    clearAll: 'Alle Löschen',
    clearAllTitle: 'Alle Bibliotheken Löschen',
    clearAllMessage: 'Möchten Sie wirklich alle installierten Bibliotheken entfernen?',
    clearAllWarning:
      'Dies entfernt alle Bibliothek-Metadaten aus dem Speicher. Verwenden Sie dies, um beschädigte Daten zu reparieren.',
    clearAllTooltip: 'Alle installierten Bibliotheken entfernen (Fehlerbehebung)',

    // Loading states
    loadingPlugins: 'Lade Bibliotheken...',
    updatesAvailable: 'Aktualisierungen verfügbar',

    // Network/Error states
    offline: 'Sie scheinen offline zu sein. Bitte überprüfen Sie Ihre Verbindung.',
    networkError: 'Verbindung zum Bibliothek-Marketplace fehlgeschlagen.',
    fetchError: 'Bibliotheken konnten nicht geladen werden. Bitte versuchen Sie es erneut.',
  },

  // AI Integration
  ai: {
    title: 'KI-Integration',
    description:
      'Verwalten Sie Ihre KI-Modelle. Fügen Sie Modelle verschiedener Anbieter hinzu und wählen Sie ein Standardmodell für intelligente Funktionen.',
    providers: {
      anthropic: 'Anthropic (Claude)',
      openai: 'OpenAI',
      google: 'Google (Gemini)',
      groq: 'Groq',
      ollama: 'Ollama (Lokal)',
    },
    status: {
      configured: 'Konfiguriert',
      notConfigured: 'Nicht konfiguriert',
    },
    fields: {
      apiKey: 'API-Schlüssel',
      apiKeyPlaceholder: 'API-Schlüssel eingeben...',
      apiKeyMasked: '••••••••••••••••',
      baseUrl: 'Basis-URL',
      baseUrlPlaceholder: 'http://localhost:11434',
    },
    actions: {
      saveKey: 'Schlüssel speichern',
      deleteKey: 'Schlüssel löschen',
    },
    messages: {
      keySaved: 'API-Schlüssel sicher gespeichert',
      keyDeleted: 'API-Schlüssel gelöscht',
      keySaveError: 'Fehler beim Speichern des API-Schlüssels',
      keyDeleteError: 'Fehler beim Löschen des API-Schlüssels',
      baseUrlUpdated: 'Basis-URL aktualisiert',
      encryptionUnavailable:
        'Sichere Verschlüsselung ist auf diesem System nicht verfügbar. Schlüssel werden mit einfacher Codierung gespeichert.',
      deleteKeyConfirm:
        'Sind Sie sicher, dass Sie diesen API-Schlüssel löschen möchten? Diese Aktion kann nicht rückgängig gemacht werden.',
      noApiKeyRequired:
        'Kein API-Schlüssel erforderlich. Stellen Sie sicher, dass der Dienst lokal läuft.',
      ollamaHint:
        'Stellen Sie sicher, dass Ollama unter dieser Adresse läuft. Standard: http://localhost:11434',
    },
    wizard: {
      title: 'Modell hinzufügen',
      step1Title: 'Anbieter auswählen',
      step1Description: 'Wählen Sie einen KI-Anbieter, um ein Modell hinzuzufügen.',
      step2Title: 'API-Schlüssel',
      step2Description: 'Konfigurieren Sie die Authentifizierung für den ausgewählten Anbieter.',
      step3Title: 'Modell auswählen',
      step3Description: 'Wählen Sie ein Modell, das Ihrer Liste hinzugefügt werden soll.',
      keyAlreadyConfigured: 'API-Schlüssel bereits konfiguriert',
      changeKey: 'Schlüssel ändern',
      saveAndContinue: 'Speichern und Fortfahren',
      fetchingModels: 'Verfügbare Modelle werden abgerufen...',
      fetchError:
        'Fehler beim Abrufen der Modelle. Überprüfen Sie Ihren API-Schlüssel und versuchen Sie es erneut.',
      noModelsFound: 'Keine Modelle für diesen Anbieter gefunden.',
      alreadyAdded: 'Bereits hinzugefügt',
      addModel: 'Hinzufügen',
      addAnother: 'Weiteres hinzufügen',
      done: 'Fertig',
      searchModels: 'Modelle suchen...',
      baseUrlLabel: 'Ollama Basis-URL',
    },
    models: {
      title: 'Modelle',
      empty:
        'Noch keine Modelle konfiguriert. Klicken Sie auf "Modell hinzufügen", um zu beginnen.',
      setDefault: 'Als Standard festlegen',
      remove: 'Modell entfernen',
      removeConfirm: 'Sind Sie sicher, dass Sie dieses Modell entfernen möchten?',
      default: 'Standard',
      addModel: 'Modell hinzufügen',
      modelAdded: 'Modell erfolgreich hinzugefügt',
      modelRemoved: 'Modell entfernt',
      defaultSet: 'Standardmodell aktualisiert',
    },
    chatDefaults: {
      title: 'Chat-Standardwerte',
      description: 'Standardeinstellungen für neue Chat-Sitzungen.',
    },
  },

  // AI Chat
  chat: {
    title: 'KI-Chat',
    emptyState: 'Starten Sie eine Unterhaltung',
    you: 'Sie',
    assistant: 'Assistent',
    thinking: 'Denkt nach...',
    showThinking: 'Denkprozess anzeigen',
    copied: 'In die Zwischenablage kopiert',
    copy: 'Kopieren',
    inputPlaceholder: 'Nachricht eingeben...',
    send: 'Senden',
    stop: 'Stopp',
    attachPdf: 'PDF anhängen',
    addFileContext: 'Datei hinzufügen',
    noOpenFiles: 'Keine geöffneten Dateien',
    sessions: {
      title: 'Sitzungen',
      newSession: 'Neue Sitzung',
      deleteSession: 'Sitzung löschen',
      renameSession: 'Sitzung umbenennen',
      deleteConfirm: 'Sind Sie sicher, dass Sie diese Sitzung löschen möchten?',
      noSessions: 'Noch keine Chat-Sitzungen',
      noSessionsHint: 'Starten Sie eine neue Unterhaltung, um zu beginnen',
      untitled: 'Unbenannte Sitzung',
      archived: 'Archiviert',
      messageCount: '{count} Nachricht | {count} Nachrichten',
      search: 'Sitzungen suchen...',
      noResults: 'Keine passenden Sitzungen',
      yesterday: 'Gestern',
      chatConfig: 'Chat-Einstellungen',
      sessionName: 'Sitzungsname',
    },
    messages: {
      user: 'Sie',
      assistant: 'Assistent',
      system: 'System',
      copyMessage: 'Nachricht kopieren',
      copyCode: 'Code kopieren',
      copied: 'In die Zwischenablage kopiert',
      branchFrom: 'Von hier verzweigen',
      regenerate: 'Antwort regenerieren',
      noMessages: 'Starten Sie eine Unterhaltung',
      noMessagesHint: 'Geben Sie unten eine Nachricht ein, um mit dem KI-Modell zu chatten',
    },
    input: {
      placeholder: 'Nachricht eingeben...',
      placeholderStreaming: 'Warte auf Antwort...',
      send: 'Senden',
      stop: 'Stopp',
      attachPdf: 'PDF anhängen',
      sendShortcut: 'Enter zum Senden, Shift+Enter für neue Zeile',
    },
    streaming: {
      thinking: 'Denkt nach...',
      showThinking: 'Denkprozess anzeigen',
      hideThinking: 'Denkprozess ausblenden',
      generating: 'Antwort wird generiert...',
      stopped: 'Antwort gestoppt',
      error: 'Fehler beim Generieren der Antwort',
      tokenUsage: '{prompt} Prompt + {completion} Vervollständigung = {total} Tokens gesamt',
    },
    waiting: {
      connecting: 'Verbindung zur KI...',
      thinking: 'Denke nach...',
      analyzing: 'Analysiere Ihre Anfrage...',
      preparing: 'Antwort wird vorbereitet...',
      processing: 'Verarbeitung...',
      almostReady: 'Fast fertig...',
    },
    memory: {
      title: 'Gedächtnisstrategie',
      buffer: 'Buffer',
      bufferDesc: 'Vollständiger Gesprächsverlauf',
      bufferWindow: 'Buffer-Fenster',
      bufferWindowDesc: 'Letzte {count} Nachrichten',
      summary: 'Zusammenfassung',
      summaryDesc: 'KI-generierte Zusammenfassung',
      summaryBuffer: 'Zusammenfassung + Buffer',
      summaryBufferDesc: 'Zusammenfassung mit aktuellen Nachrichten',
      vector: 'Vektor',
      vectorDesc: 'Ähnlichkeitsbasierte Abfrage',
      windowSize: 'Fenstergröße',
      windowSizeLabel: 'Fenstergröße',
      maxTokens: 'Maximale Tokens',
      maxTokensLabel: 'Maximale Tokens',
      strategyChanged: 'Gedächtnisstrategie aktualisiert',
    },
    pdf: {
      title: 'PDF-Referenz',
      close: 'PDF schließen',
      page: 'Seite',
      of: 'von',
      zoomIn: 'Vergrößern',
      zoomOut: 'Verkleinern',
      fitWidth: 'Breite anpassen',
      loading: 'PDF wird geladen...',
      error: 'PDF konnte nicht geladen werden',
      noDocument: 'Kein PDF geladen',
      selectFile: 'Wählen Sie eine PDF-Datei als Referenz während des Chats',
    },
    toolbar: {
      model: 'Modell',
      switchModel: 'Modell wechseln',
      sessionTitle: 'Sitzungstitel',
      editTitle: 'Titel bearbeiten',
      toggleSessions: 'Sitzungen umschalten',
      noModel: 'Kein Modell ausgewählt',
      default: 'Standard',
      noModels: 'Keine Modelle konfiguriert',
      newChat: 'Neuer Chat',
      newSession: 'Neue Sitzung',
    },
    errors: {
      sendFailed: 'Nachricht konnte nicht gesendet werden',
      streamError: 'Streaming-Fehler aufgetreten',
      sessionCreateFailed: 'Sitzung konnte nicht erstellt werden',
      modelSwitchFailed: 'Modell konnte nicht gewechselt werden',
      noModelConfigured:
        'Kein KI-Modell konfiguriert. Gehen Sie zu den Einstellungen, um ein Modell hinzuzufügen.',
      connectionFailed: 'Verbindung zum KI-Anbieter fehlgeschlagen',
    },
    artifacts: {
      copyCode: 'Code kopieren',
      codeCopied: 'Code in die Zwischenablage kopiert',
      openInEditor: 'Im Editor öffnen',
      language: 'Sprache',
      mermaidError: 'Diagramm konnte nicht gerendert werden',
    },
  },
};
